...
declare_id!("2B5VLfJujS3ZvimvuaK1ZuruiCwicGzRwcx97YdDzPbj");
...
pub mod todo_list_app {
    ...
    pub fn adding_task(ctx: Context<AddingTask>, text: String) -> Result<()> {
        let task = &mut ctx.accounts.task; // creates mutable reference to the task account
        let author = &ctx.accounts.author; // immutable reference to author account, represents the person who is adding the new task to todolist. Need this reference to obtain the public key of the author & associate it with the task being added.
